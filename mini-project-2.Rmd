...
'variable diagnosis is not tidy since the observations are not classified into two classes--M/B, other variables such as radius_mean, texture_mean, perimeter_mean, area_mean,smoothness mean, compactness mean and concavity mean are also untidy since the observations are not in sequence which makes it hard to grab information needed quickly.
Also, too many coloms may also cause mass, so tidy tibbles should be created for spacific information.
'
...
The reason is that I want to find the relation beween malignant breast mass and begign breast mass in different aspects.
...
**Research Question**: Is the fractal dimension mean of malignant breast mass significant different than that of benign breast mass?
...
<!-------------------------- Start your work below ---------------------------->
'''
t = 0.29687, p-value = 0.7667 > 0.05
They are not significant different.
'''
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
```{r}
write_csv(cancer_dat, here("cancer_dat_data", "here_exported_file.csv"))
```
<!----------------------------------------------------------------------------->
